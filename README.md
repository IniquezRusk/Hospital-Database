...
The user interface (UI) is contained within a separate file named Hospital-app. Which constains php files, CSS files and text files. The web interface is shown from our viewpoint as the admission team on how we utilize the database to make sure patients are admitted into the system. This application allows the team to perform actions like adding a patient, admitting them, and running analytical reports. 
Additinally, to run the UI, you must ensure that XAMPP is running so the web interface can connect and communicate with the MySQL database.
